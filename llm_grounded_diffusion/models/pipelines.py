...
def prepare_gligen_condition(bboxes, phrases, dtype, tokenizer, text_encoder, num_images_per_prompt):
    ...
    if n_objs > 0:
        for idx, (bboxes_item, phrases_item) in enumerate(zip(bboxes, phrases)):
            ...
            _phrase_embeddings = text_encoder(**tokenizer_inputs).pooler_output # (1, 768)
            phrase_embeddings[idx, :_phrase_embeddings.shape[0]] = _phrase_embeddings # (batch_size, max_objs, 768)
    ...
    # ********** increase dim of batch size
    boxes = boxes.repeat(repeat_times, 1, 1) # (4, 30, 4)
    phrase_embeddings = phrase_embeddings.repeat(repeat_times, 1, 1)
    masks = masks.repeat(repeat_times, 1)
    masks[:condition_len // 2] = 0 # ********** change repeated part to 0.

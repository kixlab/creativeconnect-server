...
def compose_latents(model_dict, latents_all_list, mask_tensor_list, num_inference_steps, overall_batch_size, height, width, latents_bg=None, bg_seed=None, compose_box_to_bg=True, use_fast_schedule=False, fast_after_steps=None):
    ...
    if use_fast_schedule:
        ...
        # ********** composed_latents: [9, 1, 4, 64, 64], 이 중 첫번째에는 latents_bg를 넣어줌
...
@torch.no_grad()
def compose_controlnet_latents_with_alignment(
    model_dict, latents_all_list, pre_composed_latents, mask_tensor_list, foreground_indices,  
    offset_list, num_inference_steps, overall_batch_size, height, width,
    align_with_overall_bboxes=True, overall_bboxes=None, horizontal_shift_only=False, 
    latents_bg=None, bg_seed=None, compose_box_to_bg=True, use_fast_schedule=False, fast_after_steps=None,
):
    if align_with_overall_bboxes and len(latents_all_list):
        expanded_overall_bboxes = utils.expand_overall_bboxes(overall_bboxes)
        latents_all_list, mask_tensor_list, ctrl_offset_list = align_with_bboxes(latents_all_list, mask_tensor_list, bboxes=expanded_overall_bboxes, horizontal_shift_only=horizontal_shift_only)
    else:
        ctrl_offset_list = [(0., 0.) for _ in range(len(latents_all_list))]
    # ********** add ctrl offset_list to original offset_list
    offset_list = offset_list + ctrl_offset_list

    pre_composed_latents, foreground_indices = pre_composed_latents.to('cpu'), foreground_indices.to('cpu')
    
    # ********** TODO Compose control latents only first latents. 
    unet, scheduler, dtype = model_dict.unet, model_dict.scheduler, model_dict.dtype

    # ********** pre_composed_latents already exist.
    # if latents_bg is None:
    #     generator = torch.manual_seed(bg_seed)  # Seed generator to create the inital latent noise
    #     latents_bg = get_scaled_latents(overall_batch_size, unet.config.in_channels, height, width, generator, dtype, scheduler)
    ...
    # ********** foreground_indices are mixed
    # foreground_indices = torch.zeros(latents_bg.shape[-2:], dtype=torch.long) # ********** (64,64) What means for?
    ...
    if compose_box_to_bg:
        ...
        for mask_idx in mask_order:
            ...
            mask_tensor_expanded = mask_tensor[None, None, None, ...].to(dtype)
            pre_composed_latents[0] = pre_composed_latents[0] * (1. - mask_tensor_expanded) + latents_all[0] * mask_tensor_expanded
    ...
    for mask_idx in mask_order:
        ...
        foreground_indices = foreground_indices * (~mask_tensor) + (mask_idx + 1) * mask_tensor
        mask_tensor_expanded = mask_tensor[None, None, None, ...].to(dtype)
        composed_latents = pre_composed_latents * (1. - mask_tensor_expanded) + latents_all * mask_tensor_expanded
...
def get_input_latents_list(model_dict, bg_seed, fg_seed_start, fg_blending_ratio, height, width, so_prompt_phrase_box_list=None, so_boxes=None, verbose=False):
    ...
    for idx, obj_box in enumerate(so_boxes):
        ...
        if verbose:
            ...
            # plt.show()
            plt.savefig('initial_noise.png') # ********** (64, 64)
        ...
        # ********** Latent vector is blended by each bbox.
